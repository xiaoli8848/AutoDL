...
public static class DayTimeCalculation
{
    ...
    {
        var result = DayLen(date.Year, date.Month, date.Day, longitude, latitude, -35.0 / 60.0, 1);
        return result;
    }
    ...
    public static SunTimeResult GetSunTime(DateTime date, double longitude, double latitude)
    {
        ...
        var sunrise = ToLocalTime(date, start);
        var sunset = ToLocalTime(date, end);
    ...
    {
        var hour = Convert.ToInt32(Math.Floor(utTime));
        var temp = utTime - hour;
        hour += 8; //转换为东8区北京时间
        temp = temp * 60;
        var minute = Convert.ToInt32(Math.Floor(temp));
        try
        {
